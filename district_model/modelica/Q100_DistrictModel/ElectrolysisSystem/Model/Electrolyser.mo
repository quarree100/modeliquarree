...
model Electrolyser "temperature-controlled electrolysis system"
  replaceable model CellModel = PartialModels.PartialCell constrainedby PartialModels.PartialCell "choose whether to use simple or detailed model for electrolysis" annotation(
     choices(choice(redeclare model CellModel = DetailedModel.Cell "detailed model"), choice(redeclare model CellModel = SimpleModel.Cell "simple model")));
  ...
  Q100_DistrictModel.ElectrolysisSystem.Tools.Connectors.GasConnector port_outlet_o2(redeclare package Gas = O2) annotation(
