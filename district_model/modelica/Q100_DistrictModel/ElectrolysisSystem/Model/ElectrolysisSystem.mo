...
model ElectrolysisSystem "electrolysis system with all components and heat pump"
  ...
  parameter SI.Efficiency param_transformer_eta=0.95 "constant efficiency of transformer" annotation(Dialog(group="general parameter"));
  ...
  parameter Boolean param_outputPumpMassFlow = true "output controlled mass flow rate to get desired heat pump outlet temperature" annotation(choices(checkBox=true), Dialog(descriptionLabel=true));
  ...
  SI.Pressure param_stack_p_op_cathode=1e5 "operating pressure at cathode"
                                                                          annotation(Dialog(group="electrolysis parameter"));
  SI.Pressure param_stack_p_op_anode=1e5 "operating pressure at anode"
                                                                      annotation(Dialog(group="electrolysis parameter"));
  ...
  // heat pump parameter
  parameter SI.Temperature param_hp_T_init(displayUnit="K") = 50+273.15 "initial temperature of heat pump" annotation(Dialog(group="heat pump parameter"));
  parameter SI.Temperature param_hp_T_out_cond(displayUnit="K")=82+273.15 "desired output temperature of heat pump condenser" annotation(Dialog(group="heat pump parameter", enable = param_outputPumpMassFlow));
  parameter SI.Power param_hp_P_max_el = 50e3 "maximum power consumption"
                                                                         annotation(Dialog(group="heat pump parameter"));
  parameter Real param_hp_COP = 3 "constant COP of heat pump"
                                                             annotation(Dialog(group="heat pump parameter"));
  ...
  Electrolyser comp_electrolyser(
    ...
    param_stack_mass_h2o=param_stack_mass_h2o,
    param_ppm_h2o_h2=param_ppm_h2o_h2,
    param_p_comp_h2=param_p_comp_h2,
    param_n_comp_h2=param_n_comp_h2,
    param_pump_Qmax=param_pump_Qmax,
    param_pump_dp=param_pump_dp,
    param_pump_eta=param_pump_eta) annotation (Placement(transformation(extent={{-14,24},{18,56}})));

  HeatPump.HeatPump comp_heatPump(
    param_COP = param_hp_COP,
    param_T_set_evap=param_stack_T_in,
    param_T_init = param_hp_T_init,
    param_P_el = param_hp_P_max_el)
    annotation (Placement(transformation(extent={{-20,-50},{20,-10}})));

  Modelica.Blocks.Sources.RealExpression exp_P_av_electrolyser(y=
        block_transformer.y - comp_heatPump.port_P_el.P)
    "power available for electrolyser"
    annotation (Placement(transformation(extent={{-92,44},{-72,64}})));
  Modelica.Blocks.Sources.RealExpression exp_P_av_heatpump(y=block_transformer.y)
    "power available for heat pump"
    annotation (Placement(transformation(extent={{-88,-32},{-68,-12}})));
  Tools.Connectors.PowerConnector port_P_el annotation (Placement(transformation(extent={{-140,140},
            {-120,160}}), iconTransformation(extent={{-160,120},{-120,160}})));
  Modelica.Blocks.Interfaces.BooleanInput inp_hp_on
    "if false heat pump does not turn on" annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=180,
        origin={-156,0}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={50,-170})));
  Modelica.Blocks.Interfaces.RealInput inp_P_av
    "available power for electrolysis"
    annotation (Placement(transformation(extent={{-180,60},{-140,100}}),
        iconTransformation(extent={{-160,80},{-140,100}})));
  AixLib.FastHVAC.Interfaces.EnthalpyPort_a port_inlet_cooling
  ...
  AixLib.FastHVAC.Components.Sensors.TemperatureSensor meas_T_hp_cond if param_outputPumpMassFlow
    annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=0,
        origin={34,-48})));
  AixLib.Controls.Continuous.LimPID contr_hp_cond_mflow(
    ...
    yMax=1,
    yMin=1e-5,
    y_start=0.01) if param_outputPumpMassFlow
    "mass flow controller for condenser of heat pump"
                                        annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=0,
        origin={74,0})));
  Modelica.Blocks.Interfaces.RealOutput out_hp_cond_mflow if param_outputPumpMassFlow
    " mass flow rate to get desired hp outlet temperature" annotation (
      Placement(transformation(
        ...
        rotation=270,
        origin={-50,-170})));
  Modelica.Blocks.Sources.RealExpression exp_T_set_hp_cond(y=param_hp_T_out_cond) if param_outputPumpMassFlow
    "setpoint temperature for condenser outlet"
    annotation (Placement(transformation(extent={{36,-10},{56,10}})));

  Modelica.Blocks.Logical.And block_and
    "if true heat pump turns on (assuming P_av is high enough)"
    annotation (Placement(transformation(extent={{-46,-14},{-38,-6}})));
  Modelica.Blocks.Math.Gain block_transformer(k=param_transformer_eta)
    "transformer with constant efficiency"
  ...
  Modelica.Blocks.Sources.RealExpression exp_zero(y=0) "zero value"
    annotation (Placement(transformation(extent={{-148,92},{-128,112}})));
  Modelica.Blocks.Interfaces.RealInput inp_T_air "outside air temperature"
    annotation (Placement(transformation(extent={{-180,8},{-140,48}}),
        iconTransformation(extent={{-160,40},{-140,60}})));
  Modelica.Blocks.Interfaces.RealOutput out_P_th
    "thermal power output of electrolysis system [W]"
                                                  annotation (Placement(
        transformation(extent={{140,60},{160,80}}), iconTransformation(extent={{
            140,80},{160,100}})));
  Modelica.Blocks.Interfaces.RealOutput out_P_el
    "electrical power demand of electrolysis system [W]" annotation (Placement(
        transformation(extent={{140,80},{160,100}}), iconTransformation(extent={
            {140,100},{160,120}})));
equation
 ...
 if param_outputPumpMassFlow then
    connect(contr_hp_cond_mflow.y, out_hp_cond_mflow)    annotation (Line(points={{80.6,0},
            {144,0}},                                                                                color={0,0,127}));
    connect(meas_T_hp_cond.T, contr_hp_cond_mflow.u_m) annotation (Line(points={{34.6,
            -41.4},{34,-41.4},{34,-28},{74,-28},{74,-7.2}}, color={0,0,127}));
    connect(exp_T_set_hp_cond.y, contr_hp_cond_mflow.u_s) annotation (Line(points={{57,0},{66.8,0}}, color={0,0,127}));
  ...
  // connect statements
  connect(port_inlet_cooling, comp_heatPump.port_cond_in) annotation (Line(
        points={{-126,-144},{-126,-84},{-18,-84},{-18,-48}}, color={176,0,0}));
  connect(comp_electrolyser.port_outlet_cooling, comp_heatPump.port_evap_in)
    annotation (Line(points={{12,26},{12,0},{28,0},{28,-12},{18,-12}}, color={176,0,0}));
  connect(comp_electrolyser.port_inlet_cooling, comp_heatPump.port_evap_out)
    annotation (Line(points={{-8,26},{-8,0},{-28,0},{-28,-12},{-18,-12}}, color={176,0,0}));
  connect(exp_P_av_electrolyser.y, comp_electrolyser.inp_P_av)
    annotation (Line(points={{-71,54},{-40,54},{-40,44},{-15,44}}, color={0,0,127}));
  connect(exp_P_av_heatpump.y, comp_heatPump.inp_P_av) annotation (Line(points={
          {-67,-22},{-44,-22},{-44,-24},{-22,-24}}, color={0,0,127}));
  connect(block_and.y, comp_heatPump.inp_offSwitch) annotation (Line(points={{-37.6,
          -10},{-30,-10},{-30,-16},{-22,-16}}, color={255,0,255}));
  connect(comp_electrolyser.out_cooling_system_on, block_and.u1) annotation (
      Line(points={{-15,26},{-36,26},{-36,14},{-56,14},{-56,-10},{-46.8,-10}},
        color={255,0,255}));
  connect(block_and.u2, inp_hp_on) annotation (Line(points={{-46.8,-13.2},{-60,-13.2},
          {-60,0},{-156,0}}, color={255,0,255}));
  connect(inp_P_av, block_max.u2)
    annotation (Line(points={{-160,80},{-112,80}}, color={0,0,127}));
  connect(block_max.y, block_transformer.u)
    annotation (Line(points={{-89,86},{-70,86}}, color={0,0,127}));
  connect(exp_zero.y, block_max.u1) annotation (Line(points={{-127,102},{-120,102},
          {-120,92},{-112,92}}, color={0,0,127}));
  connect(inp_T_air, comp_electrolyser.inp_T_air) annotation (Line(points={{
          -160,28},{-88,28},{-88,36},{-15,36}}, color={0,0,127}));
  connect(out_P_th, out_P_th)
    annotation (Line(points={{150,70},{150,70}}, color={0,0,127}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-160,-160},
            {160,160}}), graphics={                                                                                                                                                                                                        Ellipse(origin={17,
              45},                                                                                                                                                                                                        extent={{
              -117,107},{83,-93}},                                                                                                                                                                                                        endAngle = 360),
        Line(
          points={{-120,-148},{-120,-110},{120,-110},{120,-150}},
          color={0,0,0},
          thickness=0.5),
        Line(
          points={{90,-68},{90,-88},{-90,-88},{-90,-68}},
          color={0,0,0},
          thickness=0.5),
                     Text(origin={-23.76,35.82},
                                               extent={{-44.24,48.18},{11.76,-17.82}},
                                                                                textString = "P"),                                                                              Text(origin={
              58.66,35.82},                                                                                                                                                                              extent={{
              -42.66,48.18},{11.34,-17.82}},                                                                                                                                                                                              textString = "H"), Text(origin={
              57.0526,38.6666},                                                                                                                                                                                                        extent={{
              -5.05259,3.33338},{18.9474,-16.6666}},                                                                                                                                                                                                        textString = "2"),                                                                                Polygon(fillColor = {76, 76, 76},
            fillPattern =                                                                                                                                                                                                        FillPattern.Solid, points={{
              -10,64},{14,52},{-10,38},{-10,64}}),                                                 Text(origin={
              -46.3632,39.1428},                                                                                             extent={{
              -3.63632,2.85714},{16.3632,-17.1428}},                                                                                                        textString = "el"),
        Rectangle(
          extent={{-30,-78},{30,-120}},
          lineColor={0,0,0},
          lineThickness=0.5,
        ...
        Text(
          ...
          textString="HP"),
        Polygon(
          points={{-100,-68},{-90,-48},{-80,-68},{-100,-68}},
          lineColor={0,0,0},
          lineThickness=0.5,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid),
        Polygon(
          points={{6,-5},{-4,15},{-14,-5},{6,-5}},
          lineColor={0,0,0},
          lineThickness=0.5,
          fillColor={238,46,47},
          fillPattern=FillPattern.Solid,
